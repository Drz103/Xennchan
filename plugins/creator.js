let handler = async(m, { conn }) => {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    [`${owner[0]}`, `${await conn.getName(owner[0]+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `fiwoy13863@gmail.com`, `🇮🇩 Indonesia`, `🚀 http://tiktok.com/@notyourfavboy330`, `👤 Gada pawang nih senggol dong 😔`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://youtube.com/channel/UCL91faappuHbbUImf3aZ_2A`, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], fkonn) 
...
handler.help = ['owner']
handler.tags = ['info']
handler.command = /^(owner|creator|devoloper)$/i
