...
const language = global.lang //atau langsung lang
...
END:VCARD`.trim()
    ...
    */
    ...
    conn.reply2 = async (jid, text = '', quoted, options) => {
        return Buffer.isBuffer(text) ? this.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { ...options, text: await tr(text, {from: 'auto', to: lang}) }, { quoted, ...options })
    }
...
return await conn.sendMessage(jid, buttonMessage, {quoted})
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/" + owner[0]},
        mimetype: global.doc,
...
*/
  conn.sendTemplateButtonImg = async (jid, contentText, footer, buffer, buttons1, row1, quoted) => {
    ...
    const memek = await prepareWAMessageMedia({ image: buffer }, { upload: conn.waUploadToServer })
    const template = generateWAMessageFromContent(quoted.key.remoteJid, proto.Message.fromObject({
      templateMessage: {
        hydratedTemplate: {
          imageMessage: memek.imageMessage,
          hydratedContentText: contentText,
          hydratedFooterText: footer,
          hydratedButtons: [{
            urlButton: {
...
*/
  ...
  conn.sendTemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, options) => {
        ...
        const key = {
            ...
            ...options,
            templateButtons: [
            { index: 1, urlButton: { displayText: dtu2, url: urlnya2 } },
...
exports.smsg = (conn, m, hasParent) => {
    ...
    m.reply2 = (text, chatId, options) => conn.reply2(chatId ? chatId : m.chat, text, m, options)
