...
let handler  = async (m, { conn, command, args, usedPrefix, DevMode }) => {
    ...
    let shop = 'https://telegra.ph/file/661258deab79f78a26d8c.jpg'
    const Kchhat = `
    ...
    try {
        if (/shop|toko/i.test(command)) {
            ...
            case 'buy':
                ...
                case 'pickaxe':
                  ...
                  case 'fishingrod':
                       ...
                       break
                    default:
                        conn.send2ButtonLoc(m.chat, await (await fetch(shop)).buffer(), Kchat, wm, 'Dompet', '.my', 'Inventory', '.inv', m)
                }
                break
            case 'upgrade':
               switch (_type) {
            ...
            case 'sell': 
                ...
                break
            default:
                return conn.send2ButtonLoc(m.chat, await (await fetch(shop)).buffer(), Kchat, wm, 'Dompet', '.my', 'Inventory', '.inv', m)
            }
...
handler.help = ['shop <sell|beli> <args>', 'toko <sell|beli> <args>']
handler.tags = ['rpg']
    
handler.command = /^(shop|toko|beli|sell|jual)$/i
handler.register = true
handler.private = true
//Gak work di grup soalnya
