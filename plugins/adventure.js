...
`.trim()
            ...
            conn.sendButtonLoc(m.chat, await (await fetch(json.image)).buffer(), str.trim(), wm, 'HEAL', '.heal', m)
            if (mythic > 0) {
                   global.db.data.users[m.sender].mythic += mythic * 1
                   conn.sendBut(m.chat, '*Selamat anda mendapatkan item Rare yaitu*\n' + mythic + ' Mythic Crate', wm, 'Open', '.open mythic 1', m)
            }
            if (legendary > 0) {
                global.db.data.users[m.sender].legendary += legendary * 1
                conn.sendBut(m.chat, '*Selamat anda mendapatkan item Epic yaitu*\n' + legendary + ' Legendary Crate', wm, 'Open', '.open legendary 1', m)
            }
            } else conn.sendBut(m.chat, `Anda sudah berpetualang dan kelelahan, silahkan coba *${timers}* lagi`.trim(), wm, 'entory', '.inv', m)
        } else conn.send2But(m.chat, 'Minimal 80 health untuk bisa berpetualang, beli nyawa dulu dengan ketik *' + usedPrefix + 'shop buy potion <jumlah>*\ndan ketik *' + usedPrefix + 'use potion <jumlah>*\n\n_Untuk mendapat money dan potion gratis ketik_ *' + usedPrefix + 'claim*', wm, 'Heal', '#heal', 'Buy Potion', '.buy potion 1', m)
    } catch (e) {
        console.log(e)
