let handler = async(m, { conn, usedPrefix, command }) => {
  if (/^self$/i.test(command)) {
    ...
    if (!settings.self == true) {
      await conn.sendBut(m.chat, `Berhasil set ke ${command}!`, wm, 'Public', usedPrefix + 'public', m)
      settings.self = true
    }
  }
  if (/^publi(c|k)$/i.test(command)) {
    ...
    if (!settings.self == false) {
      await conn.sendBut(m.chat, `Berhasil set ke ${command}!`, wm, 'Self', usedPrefix + 'self', m)
      settings.self = false
    }
  }
}
