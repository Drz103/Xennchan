  ...
  try {
    ...
    if(global.db.data.users[m.sender].pasangan != "" && global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender && global.db.data.users[m.sender].pasangan != user){
      ...
      if (global.db.data.users[pacar].pasangan == user){
        ...
        conn.send2But(m.chat,`Anda baru saja mengajak @${user.split('@')[0]} berpacaran\n\nSilahkan menunggu jawaban darinya!\n\nKetik *${usedPrefix}terima @user* untuk menerima\n*${usedPrefix}tolak @user untuk menolak*`, wm, 'Terima', `.terima ${tag}`, 'Tolak', `.tolak ${tag}`, m,{contextInfo: {
          mentionedJid: [user]
        }})
      }
      ...
      global.db.data.users[m.sender].pasangan = user
      conn.send2But(m.chat,`Kamu baru saja mengajak @${user.split('@')[0]} berpacaran\n\nSilahkan menunggu jawaban darinya!\n\nKetik *${usedPrefix}terima @user* untuk menerima\n*${usedPrefix}tolak @user untuk menolak*`, wm, 'Terima', `.terima ${tag}`, 'Tolak', `.tolak ${tag}`, m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
	}	
