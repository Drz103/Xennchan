...
`.trim()
    try {
                  ...
                  case 'lobster':
                     ...
                     case 'udang':
                      ...
                      case 'cumi':
                        ...
                        break
                    default:
                        return conn.send2But(m.chat, Kchat, wm, 'Kandang', '.kandang', 'Kolam', '.kolam', m)
                }
                break
            default:
                return conn.send2But(m.chat, Kchat, wm, 'Kandang', '.kandang', 'Kolam', '.kolam', m)
      ...
      /*  } else if (/beli|buy/i.test(command)) {
        ...
        } else if (/membeli|menjual|/i.test(command)) {
            const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
            switch (type) { 
                       case 'banteng':
                        if (global.db.data.users[m.sender].banteng >= count * 1) {
...
handler.help = ['pasar <menjual> <args>']
handler.tags = ['rpg']
    
handler.command = /^(pasar|menjual)$/i
